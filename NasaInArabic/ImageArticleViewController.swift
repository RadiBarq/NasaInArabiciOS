//
//  ImageArticleViewController.swift
//  NasaInArabic
//
...
class ImageArticleViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, WKNavigationDelegate, WKUIDelegate {
    ...
    */
    ...
    func getData()
    {
        ...
        if (article != nil)
        {
            urlString = "https://api.nasainarabic.net/image/" + article!.id
            
        }
            
        else
        {
            urlString = "https://api.nasainarabic.net/image/" + articleId!
        ...
        URLSession.shared.dataTask(with: url){ (data, repnose, error) in
            DispatchQueue.main.async {
                ...
                do {
                    ...
                    let onePageArticle = try decoder.decode(CurrentImageArticleData.self, from: data)
                    ...
                    if (self.article == nil)
                    {
                       ...
                       // self.descriptionLabel.text = onePageArticle.data?.description.htmlToString
    ...
    func onClickShare()
    {
        var articleUrl =  "https://nasainarabic.net/r/i/" + (article?.id)!
        articleUrl.share()
...
public struct CurrentImageArticleData: Decodable
{
    ...
    public var data: CurrentImageArticle?

}


public struct CurrentImageArticle:Decodable{
...
public struct ImageTag: Decodable{
